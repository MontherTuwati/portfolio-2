...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const [textLines] = useState<string[]>(['', '']);
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    if (opening) {
      // Transform to X: move both lines to center and rotate
      gsap.to(plusH, { y: 0, rotate: 45, duration: 0.3, ease: 'power2.out' });
      gsap.to(plusV, { y: 0, rotate: -45, duration: 0.3, ease: 'power2.out' });
    } else {
      // Transform back to hamburger: separate lines and reset rotation
      gsap.to(plusH, { y: -6, rotate: 0, duration: 0.3, ease: 'power2.out' });
      gsap.to(plusV, { y: 6, rotate: 0, duration: 0.3, ease: 'power2.out' });
    }
  }, []);
  ...
  const animateText = useCallback(() => {
    // No text animation for hamburger menu - keeping function to maintain compatibility
    return;
  }, []);
